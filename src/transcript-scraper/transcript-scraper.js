...
//                                  you also need an Element which is p, which is an Error output field.
document.addEventListener("DOMContentLoaded", async function () {
    ...
    document.getElementById('PDF-Scraper-Input').addEventListener('change', async function(event) {
        ...
        if (!file) {
            console.log("element: 'fileInput' changed, but we havent gotten a file yet");
            return;
        }
        ...
        reader.onload = async function() {
            ...
            if(flagKTH_NeverSet){
                document.getElementById('PDF-Scraper-Error').textContent = "Error: Provided pdf doesn't contain KTH.";
                document.getElementById('PDF-Scraper-Error').style.visibility = "visible";
            }

            document.getElementById('transcript-scraper.js:output').textContent = extractedText || 'No matching text found.';
        };
